...
def test_state_store_round_trip(tmp_path):
    ...
    store.load()
    assert store.should_notify("course:1", "2026-01-01T00:00:00Z")

    store.mark_notified("course:1", "2026-01-01T00:00:00Z")
    assert not store.should_notify("course:1", "2026-01-01T00:00:00Z")
    assert store.should_notify("course:1", "2026-01-01T00:00:01Z")

    store.save()

    store2 = st.StateStore(path)
    store2.load()
    assert store2.snapshot() == {"course:1": "2026-01-01T00:00:00Z"}
